...
coinpunk.controllers.Tx.prototype.defaultFee = '0.005';
...
coinpunk.controllers.Tx.prototype.create = function() {
  ...
  if(address == '')
    ...
    try {
      ...
      errors.push('The provided ArtByte address is not valid.');
    }
  }
  ...
  for(var i=0; i<myAddresses.length;i++) {
    if(myAddresses[i].address == address)
      errors.push('You cannot send to your own ArtByte (formerly AppleByte) wallet.');
  }
  ...
  if(amount == '' || parseFloat(amount) == 0) {
    ...
    errors.push('Cannot spend more ArtByte (formerly AppleByte)  than you currently have.');
  }
...
coinpunk.controllers.Tx.prototype.scanQR = function(event) {
  ...
  qrcode.callback = function(result) {
    ...
    if(uri.protocol() != 'applebyte')
      return errorsDiv.removeClass('hidden').text('Not a valid ArtByte (formerly AppleByte) QR code.');
    
    var address = uri.path();
    if(!address || address == '')
      return errorsDiv.removeClass('hidden').text('No ArtByte (formerly AppleByte) address found in QR code.');
